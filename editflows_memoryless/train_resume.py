...
def main():
    ...
    while step < num_steps:
        ...
        for batch_x1 in loader:
            ...
            if is_main_process() and (step % save_every == 0):
                os.makedirs("checkpoints_ver2", exist_ok=True)
                ckpt_path = os.path.join("checkpoints_ver2", f"editflows_dna_{step:06d}.pt")
            ...
            if is_main_process() and (step % eval_every == 0) and (oracle_wrapper is not None):
                ...
                # =========================
                # DRAKES-style metrics
                # =========================

                # (A) Oracle metrics: 반드시 200bp로 맞춘 뒤 oracle에 넣기
                gen_oracle = [normalize_seq_random_pad(s, L=200) for s in gen_raw]

                # 2) Pred-Activity (median)  — DRAKES: HepG2 column ([:,0]) median
                acts = oracle_wrapper.predict_activity(gen_oracle, "eval")
                med_act = float(np.median(acts[:, 0])) if acts is not None and len(acts) > 0 else float("nan")

                # 3) ATAC-Acc — DRAKES: HepG2 column ([:,1]) prob>0.5 fraction (0~1)
                atac_probs = oracle_wrapper.predict_atac(gen_oracle)
                atac_idx = 1
                atac_acc = float((atac_probs[:, atac_idx] > 0.5).mean()) if atac_probs is not None and len(atac_probs) > 0 else float("nan")

                # (B) Distribution metrics: 가변 길이 "진짜 서열"로 계산 (패딩 금지)
                k3_corr_val = float("nan")
                jaspar_corr_val = float("nan")
                if reference_top01 is not None and len(reference_top01) > 0:
                    # 4) 3-mer Corr — DRAKES: top0.1% ref vs gen, 스케일 포함 Pearson
                    try:
                        k3_corr_val = float(kmer_pearson_corr(gen_raw, reference_top01, k=3, scale_mode="kmers"))
                    except Exception:
                        k3_corr_val = float("nan")

                    # 5) JASPAR Corr — DRAKES: grelu scan_sequences(...,'jaspar') + Spearman
                    try:
                        jaspar_corr_val = float(jaspar_spearman_corr(gen_raw, reference_top01, motif_db="jaspar", scale_mode="kmers"))
                    except Exception:
                        jaspar_corr_val = float("nan")
                ...
                wandb.log({
                    ...
                    "eval/ATAC-Acc": float(atac_acc),           # fraction (0~1), DRAKES 호환
                    "eval/3mer_Corr": float(k3_corr_val),
                    "eval/JASPAR_Corr": float(jaspar_corr_val),
                    "eval/App-Log-Lik_median": float("nan"),
                    # (선택) 보기 편하게 퍼센트도 추가로 남기고 싶으면:
                    # "eval/ATAC-Acc_pct": float(atac_acc * 100.0) if not np.isnan(atac_acc) else float("nan"),
                }, step=step)

                atac_print = atac_acc * 100.0 if not np.isnan(atac_acc) else float("nan")
                print(
                    f"[Eval] Act:{med_act:.3f}, ATAC:{atac_print:.1f}%, "
    ...
    if is_main_process():
        os.makedirs("checkpoints_ver2", exist_ok=True)
        ckpt_path = os.path.join("checkpoints_ver2", "editflows_dna_final.pt")
        state = (model.module.state_dict() if ddp else model.state_dict())
        torch.save({
            "model": state,
            "config": cfg,
...
CUDA_VISIBLE_DEVICES=3,4,5 torchrun --nproc_per_node=3 --master_port=29501 train_resume.py \
  --resume_ckpt /mnt/data1/intern/jeongchan/editflows_memoryless/checkpoints/editflows_dna_020000.pt
'''
