...
class SimpleEditFlowsTransformer(nn.Module):
    ...
    def forward(self, x_t: torch.Tensor, t: torch.Tensor, x_mask: torch.Tensor, return_h: bool = False) -> Dict[str, torch.Tensor]:
        """
        ...
        if return_h:
            out["h"] = h  # (B,L,D) hidden states (detached OK if caller uses no_grad)
        return out
